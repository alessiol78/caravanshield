...
{
    // Empty pointer to display buffer
    framebuff = NULL;
}

void Adafruit_GFX::constructor(int16_t w, int16_t h, bool has_gray_support)
{
    _width = WIDTH = w;
    _height = HEIGHT = h;

    framebuff_size = _width * _height ;

    support_gray_level = has_gray_support;
    // Buffer size differ from OLED type, 1 pixel is one bit
    // execpt for 96x96 seed, 1 pixel is 1 nible
    if(has_gray_support) {
        framebuff_size = framebuff_size / 2 ;
        // Init gray level for text. Default:Brightest White
        grayH= 0xF0;
        grayL= 0x0F;
    }
    else {
        framebuff_size = framebuff_size / 8;
    }
    ...
    // De-Allocate memory for frame buffer if any
    if (framebuff)
        free(framebuff);

    // Allocate memory for OLED buffer
    framebuff = (uint8_t *) malloc ( framebuff_size );
}
...
*/
    if ( support_gray_level )
    {
        // Get where to do the change in the buffer
        p = framebuff + (x + (y/2)*_width );
...
uint8_t *Adafruit_GFX::frameBuffer(int *size)
{
    if(size) {
        *size = framebuff_size;
    }
...
int16_t Adafruit_GFX::frameBufferSize()
{
...
{
    if(!framebuff) return;
    memset(framebuff, 0, framebuff_size);
}
...
void Adafruit_GFX::setGrayLevel(uint8_t grayLevel)
{
